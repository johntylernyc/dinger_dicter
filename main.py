...
from helper_functions.gbq_load_batter_statcast_data import load_data_to_gbq
...
from helper_functions.config_application_dates import most_recent_batter_date, yesterday, today_datetime, two_days_ago, batter_date_to_fetch, pitcher_date_to_fetch, today
...
def fetch_and_load_statcast_data(date_to_check, date_to_load):
    if datetime.strptime(most_recent_batter_date, '%Y-%m-%d').date() == datetime.strptime(date_to_check, '%Y-%m-%d').date():
        ...
        daily_statcast_data = get_daily_statcast_data(qualified_players, batter_date_to_fetch, date_to_load)
        ...
        load_data_to_gbq(daily_statcast_data, dataset_name, statcast_batter_table_name, json_key_path, project_id)
...
if today_datetime.hour < 9:
    fetch_and_load_statcast_data(two_days_ago, yesterday)
elif today_datetime.hour >= 9:
    fetch_and_load_statcast_data(yesterday, yesterday)
else:
